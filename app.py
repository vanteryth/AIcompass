...
def api_chat():
    ...
    if not project_id:
         return jsonify({"reply": f"⚙️ Render connected! But Dialogflow environment variables are missing.\n\nYou said: '{user_text}'\n\n(Add DIALOGFLOW_PROJECT_ID to your Render environment variables.)"})
    ...
    try:
        ...
        return jsonify({"reply": f"⚠️ Dialogflow Auth Error: {str(e)}\n\n(Make sure GOOGLE_APPLICATION_CREDENTIALS is set properly in Render.)"})
...
if __name__ == '__main__':
    # Binds to the port Render provides, defaulting to 5000 for local testing
